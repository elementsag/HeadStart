...
contract HeadStartStaking {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "HeadStartStaking: not owner");
        _;
    }

    modifier onlyInitialized() {
        require(initialized, "HeadStartStaking: not initialized");
        _;
    }
    ...
    function initialize(
        ...
        require(!initialized, "HeadStartStaking: already initialized");
        require(_token != address(0), "HeadStartStaking: zero token");
        require(_totalRewards > 0, "HeadStartStaking: zero rewards");
    ...
    function stake(uint256 _amount) external onlyInitialized updateReward(msg.sender) {
        require(_amount > 0, "HeadStartStaking: zero amount");
    ...
    function unstake(uint256 _amount) external onlyInitialized updateReward(msg.sender) {
        require(_amount > 0, "HeadStartStaking: zero amount");
        require(stakedBalance[msg.sender] >= _amount, "HeadStartStaking: insufficient balance");
    ...
    function claimReward() external onlyInitialized updateReward(msg.sender) {
        ...
        require(reward > 0, "HeadStartStaking: no reward");
    ...
    function transferOwnership(address _newOwner) external onlyOwner {
        require(_newOwner != address(0), "HeadStartStaking: zero address");
